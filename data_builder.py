...
class BertData():
    def __init__(self, args):
       ...
       ## self.tokenizer = BertTokenizer.from_pretrained('bert-base-uncased', do_lower_case=True)
        self.tokenizer = XLNetTokenizer.from_pretrained('xlnet-large-cased', do_lower_case=False)
        self.sep_token = '<sep>'
        self.cls_token = '<cls>'
        self.pad_token = '<pad>'
        self.tgt_bos = '<s>'
    ...
    def preprocess(self, src, tgt, sent_labels, use_bert_basic_tokenizer=False, is_test=False):
        ...
        src_subtokens = self.cls_token + text + self.sep_token
        #src_subtoken_idxs = self.tokenizer.convert_tokens_to_ids(src_subtokens)
        src_subtoken_idxs = self.tokenizer.encode(src_subtokens)
        ...
        #tgt_subtokens_str = '<s> ' + ' <eop> '.join(
        #    [' '.join(self.tokenizer.tokenize(' '.join(tt))) for tt in tgt]) + ' </s>'
        tgt_sub = tgt[:self.args.max_tgt_ntokens]
        tgt_subtokens_str = '<s> ' + ' <eop> '.join([' '.join(tt) for tt in tgt_sub]) + ' </s>'
        #tgt_subtoken = tgt_subtokens_str.split()[:self.args.max_tgt_ntokens]

     #   if ((not is_test) and len(tgt_sub) < self.args.min_tgt_ntokens):
     #       return None

        tgt_subtoken_idxs = self.tokenizer.encode(tgt_subtokens_str)
